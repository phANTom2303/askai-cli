...
char* get_api_key() {
    ...
    int dir_len = snprintf(config_dir, sizeof(config_dir), "%s/.askai-cli", home_dir);
    if (dir_len >= sizeof(config_dir)) {
        fprintf(stderr, "Error: Config directory path too long.\n");
        return NULL;
    }
    
    int path_len = snprintf(config_path, sizeof(config_path), "%s/config", config_dir);
...
int main() {
    printf("Attempting to get API key...\n");

    char* api_key = get_api_key();

    if (api_key) {
        printf("Success! API key obtained: ...%s\n",
               strlen(api_key) > 4 ? api_key + strlen(api_key) - 4 : api_key);

        // Use the key here...

        free(api_key);  // Free the memory
    } else {
        fprintf(stderr, "Failed to obtain API key.\n");
        return 1;
    }
